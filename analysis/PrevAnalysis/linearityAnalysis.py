...
def plotThings(M,Rid,Did,Bothid,Evenid,Oddid,IX,IXE,IXO):
    ...
    plt.subplot(121)
    for id in Rid:
        plt.loglog(freq[IX],np.absolute(REF[id]['OUTS'][IX]),'r')
        plt.loglog(freq[IX],np.absolute(REF[id]['REFS'][IX]),'b')


    plt.subplot(122)
    for id in Rid:
        plt.semilogx(freq[IX],np.angle(REF[id]['OUTS'][IX]),'r')
        plt.semilogx(freq[IX],np.angle(REF[id]['REFS'][IX]),'b')
